...
exports.matchVirtual = function (_url, root, virtual) {
  ...
  for (var v in virtual) {
    varr.push(v);
    temp = _url.match(new RegExp("^\\"+v));
    if (temp && temp[0].length > len) {
      matched = v;
      len = temp[0].length;
    }
  }
  ...
  if (matched) {
    return path.join(RT, _url.replace(new RegExp("^\\"+matched), virtual[matched]));
  }
  else {
    for (var i = 0; i < varr.length; i++) {
      ...
      else if (path.dirname(varr[i]) == path.dirname(_url) && !virtual[varr[i]].match(/^\.{2,}/)) {
