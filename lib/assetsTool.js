...
AssetsTool.prototype = {
  ...
  clear: function (str) {
    var inlineRegx = /inline=(['"])true\1/;
    var feMoveRegx = /fe\-move=(["'])[^"']*?\1/;

    return str.replace(inlineRegx, '').replace(feMoveRegx, '').replace(/\s{1,}/g, ' ').replace(/(^\s*)|(\s*$)/g, '');
  },
  extractScripts: function () {
    this.content = this.content.replace(/<script([^>]*?)>[\s\S]*?<\/script>/g, (function (all, attr) {
      var srcRegx = /src=(['"])([^"']*?)\1/;

      var matched = attr.match(srcRegx);
      if (matched && matched[2]) {
        if (/inline=(['"])true\1/.test(attr)) {
          attr = this.clear(attr.replace(srcRegx, ''));
          return "<script " + attr + ">\n" +
            '<!--\#remote url="' + matched[2] + '"-->' +
            "\n</script>";
        }
        else {
  ...
  extractStyles: function () {
    this.content = this.content.replace(/<link([^>]*?)>/g, (function (all, attr) {
      var relRegx  = /rel=(['"])stylesheet\1/;
      var hrefRegx = /href=(['"])([^"']*?)\1/;

      var matched = attr.match(hrefRegx);
      if (matched && matched[2]) {
        if (/inline=(['"])true\1/.test(attr)) {
          attr = this.clear(attr.replace(/\s{0,}\/\s{0,}$/, '').replace(hrefRegx, '').replace(relRegx, ''));
          return "<style " + attr + ">\n" +
            '<!--\#remote url="' + matched[2] + '"-->' +
            "\n</style>";
        }
        else {
          return this.rough_classify(all, matched[2], "styles");
        }
      }
      else {
    ...
    var urlResolve = function (all, $1, Q, url, $4) {
      if (typeof this.cdnPath == "string") {
        ...
        if (url.indexOf(selfRepo) != -1 && ((selfRepo == '' && !isAbs) || selfRepo != '')) {
          ...
          if (this.assetsFlag) {
            url = url.replace(/\.css$|\.css(\,)|\.less$|\.less(\,)|\.scss$|\.scss(,)/g, this.css + "$1$2$3");
