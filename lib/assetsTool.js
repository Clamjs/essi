...
function AssetsTool(realpath, param, assetsFlag) {
  ...
  this.css = param.css;
  this.js = param.js;
  this.charset = param.charset || "utf-8";
...
AssetsTool.prototype = {
  ...
  combo: function (type) {
    var config = {
      scripts: [
        ...
        function (src) {
          return '<script type="text/javascript" charset="' + this.charset + '" src="' + (src.indexOf('$') == 0 ? '' : this.cdnPath) + src + '"></script>';
        }.bind(this)
      ],
      styles: [
        /<link[^>]*? fe\-group=(['"])([^"']*?)\1[^>]*?>\n{0,}/g,
        /<link[^>]*? href=(['"])([^"']*?)\1[^>]*?>/,
        function (href) {
          return '<link type="text/css" rel="stylesheet" href="' + (href.indexOf('$') == 0 ? '' : this.cdnPath) + href + '">';
        }.bind(this)
      ]
    };
    ...
    this.content = this.content.replace(tag[0], (function (all, $1, group) {
      ...
      if (matched && matched[2] && (matched[2].indexOf(this.cdnPath) != -1 || matched[2].indexOf('$') == 0)) {
        if (!arr[group]) {
          arr[group] = [];
        }
        arr[group].push(matched[2].replace(this.cdnPath, ''));
  ...
  action: function (content) {
    ...
    function urlResolve(all, $1, Q, url, $4) {
      ...
      if (scriptRegx.test($1) && !(charsetRegx.test($1) || charsetRegx.test($4))) {
        $1 = $1.replace(scriptRegx, function (all, tag) {
          return tag + ' charset="' + this.charset + '" ';
        }.bind(this));
      }
      ...
      if (/^https?:\/\/|^\/\/|^\$\!?\{[^\}]*?\}/.test(url)) {
        ...
        if (this.cdnPath && /^https?:\/\/|^\/\//.test(this.cdnPath)) {
