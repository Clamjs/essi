...
function AssetsTool(realpath, param) {
  ...
  this.cdnPath = null;
  if (param.cdnPath) {
    this.cdnPath = urlLib.resolve(param.cdnPath.replace(/([^\/])$/, "$1/"), param.version || '');
  }
...
AssetsTool.prototype = {
  ...
  action: function (content) {
    ...
    function urlResolve($0, $1, $2, $3, $4) {
      if (self.cdnPath && !$3.match(/^http:\/\//)) {
        $3 = $3.replace(/\.less$|\.less\.css$|\.less(\,)|\.less\.css(\,)/g, self.css + "$1$2");
        $3 = $3.replace(/\.js($)|\.js(\,)/g, self.js + "$1$2");
