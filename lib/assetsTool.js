...
AssetsTool.prototype = {
  ...
  combo: function (type) {
    var config = {
      scripts: [
        ...
        function (src) {
          return '<script type="text/javascript" charset="' + this.assetsCharset + '" src="' + (src.indexOf('$') == 0 ? '' : this.cdnPath) + src + '"></script>';
        }.bind(this)
      ],
      styles: [
        /<link[^>]*? fe\-group=(['"])([^"']*?)\1[^>]*?>\n{0,}/g,
  ...
  action: function (content, resolving) {
    ...
    var urlResolve = function (all, $1, Q, url, $4) {
      if (/^https?:\/\/|^\/\//.test(url) || typeof this.cdnPath != "string") {
        return addCharset(all);
      }
      else {
        $1 = addCharset($1, $4);
        ...
        if (this.assetsFlag && url.indexOf(selfRepo) != -1) {
