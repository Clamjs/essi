  ...
  combo: function (type) {
    var config = {
      scripts: [
        ...
        function (src) {
          return '<script type="text/javascript" charset="' + this.charset + '" src="' + this.cdnPath + src + '"></script>';
        }.bind(this)
      ],
      styles: [
        /<link[^>]*? fe\-group=(['"])([^"']*?)\1[^>]*?>\n{0,}/g,
        /<link[^>]*? href=(['"])([^"']*?)\1[^>]*?>/,
        function (href) {
          return '<link type="text/css" rel="stylesheet" href="' + this.cdnPath + href + '">';
        }.bind(this)
      ]
    };
    ...
    this.content = this.content.replace(tag[0], (function (all, $1, group) {
      ...
      if (matched && matched[2] && matched[2].indexOf(this.cdnPath) != -1) {
        if (!arr[group]) {
          arr[group] = [];
        }
