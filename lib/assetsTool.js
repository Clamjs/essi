...
function AssetsTool(realpath, param) {
  this.fakePath = "http://fake_host";
  this.cdnPath = (param.cdnPath||'/').replace(/([^\/])$/, "$1/");
  this.from = realpath.replace(pathLib.join(process.cwd(), param.rootdir||"src"), this.fakePath);

  this.min = param.min || false;
  this.css = param.css || ".min.css";
  this.js  = param.js  || ".min.js";
}
AssetsTool.prototype = {
  ...
  action: function(content) {
    ...
    function urlResolve($0, $1, $2, $3, $4) {
      if (self.min && !$3.match(/^http:\/\//)) {
        $3 = $3.replace(/\.less$|\.less\.css$/, self.css||".min.css");
        $3 = $3.replace(/\.js$/, self.js||".min.js");
      }
    ...
    if (this.cdnPath != '/') {
      content = content.replace(/(<script[^>]*? src=(['"]))([^"']*?)(\2.*?>[\s\S]*?<\/script>)/g, urlResolve);
      content = content.replace(/(<link[^>]*? href=(['"]))([^"']*?)(\2.*>)/g, urlResolve);
    }
...
AssetsTool.prototype = {
  constructor: AssetsTool,
  input: function (content, opt) {
    this.content = content.replace(/\<\!\-\-^#.*\-\-\>/g, '');

    var self = this;
    function urlResolve($0, $1, $2, $3, $4) {
      if (opt.min && !$3.match(/^http:\/\//)) {
        $3 = $3.replace(/\.less$|\.less\.css$/, opt.css||".min.css");
        $3 = $3.replace(/\.js$/, opt.js||".min.js");
      }
      var url = urlLib.resolve(self.from, $3).replace(self.fakePath+'/', self.cdnPath);
      return $1+url+$4;
    }
...
EXP_AssetsTool.prototype = {
  ...
  action: function (content, opt, radical) {
    this.fe.input(content, opt);
