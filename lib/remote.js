...
var remoteREGX = '<!--\\s{0,}#remote[^->]*?url=(["\'])(https?://[^"\']*?)\\1[^->]*?-->';

function getCacheFilePath(url) {
  return pathLib.join(cacheDir, mace.MD5(url));
}
      ...
      for (var i in m) {
        (function (path, i) {
          ...
          if (cache) {
            ...
            var host = urlLib.parse(path).host;
            fetchUrl(
              (self.hosts[host] ? path.replace(host, self.hosts[host]) : path),
              {
