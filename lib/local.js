...
function Local(realpath, root, remote, traceRule) {
...
Local.prototype = {
  ...
  parse: function (str, realPath, enable) {
    var self = this;
    var SSIList = [];
  ...
  fetch: function (realPath, vars, enable) {
    var state = false;
    ...
    if (state && state.isDirectory()) {
      ...
      return this.parse(this.readFile(realPath, vars, isMain), realPath, enable);
    }
  },
...
function EXP_Local(realpath, root, remote, traceRule) {
  this.realpath = realpath;
  this.local = new Local(realpath, root, remote, traceRule);
}
EXP_Local.prototype = {
  constructor: EXP_Local,
  fetch: function (enable) {
    return this.local.fetch(this.realpath, undefined, enable);
  },
  parse: function (content, enable) {
    this.local.setRoot(this.realpath);
    content = this.local.clearMock(content, true);
    content = this.local.parse(content, this.realpath, enable);
    return content;
  }
};
